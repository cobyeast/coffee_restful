"""
Coffee_Restful: A Simple Flask CRUD API
@version: v.1.0.1
...
def load_routes():
    ...
    jwt = JWT(app, authenticate, idenity)  # sets to path /auth by default

    # Lists all api endpoints, with <:> values set as params

    app.add_url_rule('/api/items/<string:name>',
                     view_func=Item.as_view('item'))
    app.add_url_rule('/api/items/<int:_id>',
                     view_func=ItemEdit.as_view('itemedit'))
...
if __name__ == '__main__':
    ...
    load_routes()  # load all API endpoints
